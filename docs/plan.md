...
| # | やること | 成果物 | 確認方法 |
|---|---|---|---|
| **0** | **プロジェクト初期化** | `.gitignore`（`.pio/`, `.vscode/` 等）、このプラン書類を `docs/plan.md` としてリポジトリに保存 | `git status` に `.pio/` 等が出ない、`m5_basic_2_7_attracts/docs/plan.md` が残る |
| 1 | **State 構造体 + LCD 描画 + プロジェクトひな形** | `platformio.ini`, `state.hpp`（RobotStatus 含む完全版）, `lcd_view.hpp` 実装, `main.cpp` の setup/loop 骨格（ダミー値を動かす） | `pio run --target upload` で画面が出る。`s.cmd.vx = sinf(t)` 等で数値が滑らかに動く。Robot 欄は `---` |
| 2 | **モード切替ボタン** | BtnA/B/C → `s.mode` 更新 | ヘッダ色・文字が切り替わる |
| 3 | **Serial1/2 `begin` + Serial1 受信スケルトン** | setup 内で両 UART init、`Serial1.available()` をループで読んで `s.comms.rx1_bytes += n`（データは破棄） | USB-UART から GPIO36 に何か流すと LCD の RX1 バイト数が増える |
| 4 | **Reference パケット 100Hz 送信** | `crc.hpp`（XOR 実装のみ）, `reference_packet.hpp`, 10 ms タイマ送信, `tx_hz`/`tx_count` 計測 | ロジアナで `A5 5A ... CRC 00` の 28B が 10ms 周期。LCD に `TX 100.0 Hz (#count)` |
| 5 | **Bluepad32 接続 + ゲームパッドマッピング** | `gamepad_mode.hpp`, Connect/Disconnect コールバック | コントローラ操作で LCD の `vx/yaw/...` が追随、beta_main 実機で動く |

**各フェーズで独立にコミット可能**。P1–P2 完了時点で「内部状態を可視化するダッシュボード」が単独で動作するので、後続は見ながら追加できる。
...
├── platformio.ini              # ESP32 + Arduino + M5Stack + Bluepad32         [P1]
├── src/
│   └── main.cpp                # setup / loop / モード管理 / 全体統合            [P1→徐々に肥える]
└── include/
    ├── state.hpp               # State, Command, GamepadStatus, RobotStatus    [P1]
    ├── lcd_view.hpp            # lcd_init / lcd_draw(const State&)             [P1]
    ├── crc.hpp                 # CRC8 XOR / CRC8 poly / CRC16 CCITT            [P4]
    ├── reference_packet.hpp    # rabcl Reference（28B, MSB float, XOR CRC8）   [P4]
    ├── gamepad_mode.hpp        # Bluepad32 → State.cmd/gp                      [P5]
    ├── transceiver_frame.hpp   # 0xAE フレームパーサ（スケルトン→実装）        [P3→将来]
    └── transceiver_mode.hpp    # Decoded → State.cmd/robot                     [将来]
```

末尾 `[Px]` は上記の実装フェーズ番号。P1 の段階で `state.hpp` / `lcd_view.hpp` は **将来のフィールドも含めた完全版**を作ってしまい、後続フェーズでの変更範囲を狭める。
