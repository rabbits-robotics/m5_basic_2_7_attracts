...
| 種別 | 例 | テスト方式 |
|---|---|---|
| 純粋ロジック | `crc.hpp`, `reference_packet.hpp`, `labels.hpp`, `transceiver_frame.hpp`, gamepad マッピング | ホスト native テスト（Unity、必要なら ArduinoFake 追加） |
...
| 1b | **native テスト環境構築** | 📝 | `platformio.ini` に `[env:native]` (Unity)、`test/test_sanity/test_main.cpp` で sanity スイート | `pio test -e native` が 1 件 sanity で通る |
| 1c | State 構造体追加 | 📝 | `include/state.hpp`（RobotStatus 含む完全版）、`main.cpp` で宣言 | `pio run` 成功 |
| 1d | **TDD: labels.hpp** | ◎ | test 先行 → `include/labels.hpp`（`mode_label(Mode)`, `load_label(uint8_t)`, `fire_label(uint8_t)`, `speed_label(uint8_t)`, `chassis_label(uint8_t)`, `type_label(uint8_t type, bool present)`, `team_label(uint8_t team, bool present)`） | ラベル変換テストが green（`pio test -e native -f test_labels`） |
...
├── include/
│   ├── state.hpp               # State, Command, GamepadStatus, RobotStatus    [P1c]
│   ├── labels.hpp              # ラベル変換（TDD）                              [P1d]
...
└── test/                                    # `pio test -e native` が配下の `test_*/` を全スイートとして実行
    ├── test_sanity/test_main.cpp            # スモーク用 sanity                  [P1b]
    ├── test_labels/test_main.cpp            # labels.hpp テスト                  [P1d]
    ├── test_crc/test_main.cpp               # crc.hpp テスト                     [P4a]
    ├── test_reference_packet/test_main.cpp  # reference_packet.hpp テスト        [P4b]
    └── test_gamepad_mapping/test_main.cpp   # gamepad 純粋マッピングテスト       [P5a]
```
...
; lib_deps:
;   fabiobatsilva/ArduinoFake  ; Arduino API モックが要る段になったら追加
build_flags =
    -std=gnu++17
```

**ホスト gcc/g++ が必要**（Windows なら `choco install mingw` などで MinGW-w64 を導入）。
